...
def gradient(image, d=1, verbose=False):
    ...
    for i in range(d):
        x = cv2.convertScaleAbs(cv2.Sobel(d_img, cv2.CV_64F, 1, 0, ksize=-1))
        y = cv2.convertScaleAbs(cv2.Sobel(d_img, cv2.CV_64F, 0, 1, ksize=-1))
...
# np.inf for max
def shades_of_grey(image, derivative=0, lp_order=6, gaussian_size=5):
    ...
    for c in range(3):
        for y in range(0, grid[1]):
            for x in range(0, grid[0]):
                tile = smoothed_im[y*step_y:min((y+1)*step_y, im_height), x*step_x:min((x+1)*step_x, im_width), c]
                if verbose:
